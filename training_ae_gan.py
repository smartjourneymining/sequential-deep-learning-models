...
def seq_ae_predict(seq_ae_teacher_forcing_ratio, model, model_input_x, model_input_y, temperature=1.0, top_k=None, sample=False):
    ...
    if use_teacher_forcing:
        ...
        for i in range(model_input_y[0].size(1)):
            ...
            input_position = (utils.generate(a_inp, temperature=temperature, top_k=top_k, sample=sample), t_inp)
        ...
        for prefix in log_with_prefixes[subset + '_prefixes_and_suffixes']['activities']['prefixes'].keys():
            ...
            for i in range(nb_iterations):
                ...
                if subset == 'training':
                    ...
                    loss_real = adversarial_criterion(prediction_real, torch.ones(size=prediction_real.size(), device=prediction_real.device))
                    # let's speed it up:
                    # loss_real.backward()
                    loss_fake = adversarial_criterion(prediction_fake, torch.zeros(size=prediction_fake.size(), device=prediction_fake.device))
                    # loss_fake.backward()
                    (loss_real + loss_fake).backward()
                    optimizer_discriminator.step()
                    soft_sampled_prediction_categories_for_generator = torch.nn.functional.gumbel_softmax(logits=prediction[0], tau=t, hard=False)
                    prediction_fake = discriminator((soft_sampled_prediction_categories_for_generator, prediction[1]))
                    adversarial_loss = adversarial_criterion(prediction_fake, torch.ones(size=prediction_fake.size(), device=prediction_fake.device))
                ...
                if len(prediction) > 1:
                    ...
                    if subset == 'training':
                        (categorical_loss + lagrange_a * regression_loss + lagrange_b * adversarial_loss).backward()
...
def main(args, dt_object):
    ...
    for log_name in logs:
        ...
        model_generator = models.SequentialAutoEncoder(hidden_size=args.hidden_dim,
        ...
        model_discriminator = models.SequentialDiscriminator(hidden_size=args.hidden_dim,
                                                             num_layers=args.n_layers,
                                                             dropout_prob=args.dropout_prob,
                                                             vocab_size=attributes_meta[0]['vocabulary_size'],
                                                             attributes_meta=attributes_meta,
        ...
        for e in range(args.nb_epoch):
            ...
            if total_validation_loss < min_loss_threshold:
                ...
                model_to_save['optimizer_state_dict'] = copy.deepcopy(optimizer[0].state_dict())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--training_batch_size', help='number of training samples in mini-batch', default=1152, type=int)
    parser.add_argument('--validation_batch_size', help='number of validation samples in mini-batch', default=1152, type=int)
    ...
    parser.add_argument('--lagrange_a', help='Langrange multiplier a', default=1.0, type=float)
    parser.add_argument('--lagrange_b', help='Langrange multiplier b', default=1.0, type=float)
    ...
    parser.add_argument('--seq_ae_teacher_forcing_ratio', help='seq_ae_teacher_forcing_ratio', default=0.7, type=float)
